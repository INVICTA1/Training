...
def extract_txt_from_pdf(pdf_path):
    ...
    page_interpreter = PDFPageInterpreter(resource_manager, converter)#создаем объект интерпритаторов который извлечет текст
...
if __name__ == '__main__':
    with  io.open('txt.txt', 'w+', encoding="utf-8") as txt_file:
        ...
        text = re.split('[.] ', extract_txt_from_pdf(r'..\Books\fw9.pdf'))
        for offer in text: # the name od variable is wrong. Would be better ti use 'sentence'
            txt_file.write(str(offer)+'.\n')